  ...
  useEffect(() => {
    ...
    if (user && token && user.role === 'customer') {
      ...
      if (!websocketService.ws || websocketService.ws.readyState === WebSocket.CLOSED) {
        ...
        console.log('✅ CustomerWebSocketBridge: WebSocket already connected, state:', websocketService.ws.readyState);
      }
    } else {
      console.log('⚠️ CustomerWebSocketBridge: Not connecting - missing requirements');
