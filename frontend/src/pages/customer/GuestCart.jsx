...
const GuestCart = () => {
  ...
  const handleGuestCheckout = async () => {
    ...
    try {
      ...
      // เก็บ Temporary ID ใน localStorage สำหรับการติดตาม
      const guestOrders = JSON.parse(localStorage.getItem('guest_orders') || '[]');
      const newGuestOrder = {
        temporary_id: orderResult.temporary_id,
        order_date: new Date().toISOString(),
        restaurant_name: restaurantCount > 1 ? 'Multi-Restaurant Order' : (cartItems[0]?.restaurant_name || 'Unknown Restaurant'),
        total_amount: total,
        status: 'pending'
      };
      ...
      let successMessage = `Order successful!\nTemporary ID: ${orderResult.temporary_id}\nTotal: ${formatCurrency(total)}`;
      successMessage += `\n\nPlease save your Temporary ID to track your order.`;

      toast.success(successMessage);
      
      // ไปหน้า guest orders
      navigate(`/guest-orders?temporary_id=${orderResult.temporary_id}`);
          ...
          <div className="lg:col-span-2">
            ...
            <div className="bg-white rounded-lg shadow-md p-6 mb-6">
              <h3 className="text-lg font-semibold text-secondary-700 mb-4">
                {translate("cart.customer_information")}
              </h3>
            ...
            <div className="bg-white rounded-lg shadow-md p-6 sticky top-8">
              ...
              >
                {loading
                  ...
                  : !customerPhone.trim()
                  ? translate("cart.please_enter_your_phone")
                  : !proofOfPayment
