...
const CreatePhoneOrder = () => {
  ...
  const fetchAllMenuItems = useCallback(async () => {
    ...
    try {
      ...
      for (const restaurant of restaurantsList) {
        try {
          const menuResponse = await api.get(`/restaurants/${restaurant.restaurant_id}/products/`);
          ...
          console.error(`Error fetching menu for restaurant ${restaurant.restaurant_id}:`, error);
        }
      }
  ...
  const addToCart = (product) => {
    ...
    const restaurantInfo = restaurants.find(r => r.restaurant_id === product.restaurant_id);
    ...
    if (existingItem) {
      setCart(prev => prev.map(item =>
        item.product_id === product.product_id && item.restaurant_id === product.restaurant_id
      ...
      const cartItem = {
        ...
        restaurant_name: product.restaurant_name || restaurantInfo?.name || restaurantInfo?.restaurant_name || 'ไม่มีชื่อร้าน'
      };
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      console.log('Sending guest order data:', JSON.stringify(orderData, null, 2));
      ...
      alert('สร้างออร์เดอร์สำเร็จแล้ว!');
      ...
      let errorMessage = 'เกิดข้อผิดพลาดในการสร้างออร์เดอร์: ';
      ...
      <div className="bg-white/80 backdrop-blur-xl border-b border-white/20 shadow-lg shadow-green-500/5">
        ...
        <div>
                <h1 className="text-3xl font-bold bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">
                  📞 สร้างออร์เดอร์ทางโทรศัพท์
          </h1>
                <p className="text-gray-500 text-sm font-medium">สำหรับลูกค้าที่โทรมาสั่งโดยตรง - ง่าย รวดเร็ว ครบครัน</p>
              </div>
        </div>
        ...
        >
                ...
                <span className="font-medium">กลับไปหน้าออร์เดอร์</span>
        </button>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="flex items-center space-x-3 mb-6">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-800">👤 ข้อมูลลูกค้า</h2>
              <p className="text-sm text-gray-500">กรอกข้อมูลลูกค้าที่โทรมาสั่ง</p>
            </div>
          </div>
          ...
          <div className="mb-4">
            <label className="flex items-center space-x-2 text-sm font-medium text-gray-700 mb-2">
              ...
              <textarea
                ...
                rows={3}
              className="w-full p-4 bg-white/60 border border-white/30 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 focus:bg-white/80 transition-all duration-300 placeholder-gray-500"
              placeholder="กรอกที่อยู่จัดส่งแบบละเอียด (บ้านเลขที่, ซอย, ถนน, เขต/อำเภอ, จังหวัด)"
              />
            </div>
          ...
          <div>
            <label className="flex items-center space-x-2 text-sm font-medium text-gray-700 mb-2">
              ...
              <textarea
                ...
                rows={2}
              className="w-full p-4 bg-white/60 border border-white/30 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 focus:bg-white/80 transition-all duration-300 placeholder-gray-500"
              placeholder="หมายเหตุเพิ่มเติม เช่น ขอเผ็ดน้อย, ไม่ใส่ผักชี, จัดส่งช่วงเวลาใด (ถ้ามี)"
              />
            </div>
          </div>
        ...
        <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6 mb-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-800">🍽️ เมนูอาหารทั้งหมด</h2>
                <p className="text-sm text-gray-500">ค้นหาและเลือกเมนูที่ลูกค้าต้องการ ({allMenuItems.length} รายการ)</p>
              </div>
              </div>
            </div>
          ...
          <div className="relative group mb-6">
              ...
              placeholder="🔍 ค้นหาเมนู หรือ ชื่อร้าน... (พิมพ์ชื่อเมนูหรือร้านอาหารที่ต้องการ)"
            ...
            />
            <svg className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400 group-focus-within:text-orange-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
            {searchTerm && (
              <button
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 font-medium">กำลังโหลดเมนูจากทุกร้าน...</p>
              </div>
            ...
            <div className="max-h-96 overflow-y-auto space-y-3 pr-2">
              {getFilteredMenuItems.map(item => {
                ...
                return (
                  <div key={`${item.product_id}-${item.restaurant_id}`} 
                       className={`group bg-white/60 backdrop-blur-sm rounded-xl border transition-all duration-300 hover:shadow-lg hover:border-orange-300 hover:-translate-y-0.5 overflow-hidden ${
                         isInCart ? 'border-green-300 bg-green-50/80' : 'border-white/30'
                       }`}>
                    <div className="p-4">
                    <div className="flex gap-4">
                      ...
                      <div className="flex-shrink-0">
                        ...
                        <div 
                            className={`w-20 h-20 bg-gradient-to-br from-orange-100 to-red-100 rounded-xl flex items-center justify-center shadow-md ${item.image ? 'hidden' : 'flex'}`}
                        >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start mb-2">
                          <div className="flex-1">
                              ...
                              {item.product_name || item.name || 'ไม่มีชื่อเมนู'}
                            </h3>
                              <div className="flex items-center space-x-2 mt-1">
                                ...
                                <p className="text-sm text-blue-600 font-medium">
                                  {restaurant?.name || restaurant?.restaurant_name || 'ไม่มีชื่อร้าน'}
                            </p>
                          </div>
                            </div>
                            ...
                            {formatPrice(item.price)}
                              ...
                              {isInCart && (
                                <div className="flex items-center justify-end space-x-1 mt-1">
                                  ...
                                  <span className="text-xs text-green-600 font-medium">ในตะกร้า</span>
                                </div>
                              )}
                            </div>
                        </div>
                        ...
                        <button
                          ...
                          >
                            ...
                            <span>{isInCart ? 'เพิ่มอีก' : 'เพิ่มใส่ตะกร้า'}</span>
                        </button>
                        </div>
                      </div>
                    </div>
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-800 mb-2">
                {searchTerm ? 'ไม่พบเมนูที่ค้นหา' : 'ไม่มีเมนูในระบบ'}
              </h3>
              <p className="text-gray-500">
                {searchTerm 
                  ? `ลองค้นหาด้วยคำอื่น หรือตรวจสอบการพิมพ์` 
                  : 'กรุณาเพิ่มเมนูในระบบก่อนสร้างออร์เดอร์'
                }
              </p>
            </div>
          )}
          ...
          <div className="xl:col-span-1">
            ...
            <div className="flex items-center space-x-2 mb-4">
              ...
              <div>
                <h2 className="text-lg font-bold text-gray-800">🛍️ ตะกร้าสินค้า</h2>
                <p className="text-xs text-gray-500">{cart.length} รายการ</p>
              </div>
            </div>
            ...
            {cart.length > 0 ? (
              <div className="space-y-3 max-h-64 overflow-y-auto pr-2">
                {cart.map((item, index) => {
                  ...
                  return (
                  <div key={`${item.product_id}-${item.restaurant_id}`} className="bg-white/60 rounded-lg p-3 border border-white/30">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex-1 min-w-0">
                        <h4 className="font-medium text-gray-800 text-sm truncate">{item.product_name || 'ไม่มีชื่อสินค้า'}</h4>
                        <div className="flex items-center space-x-1 mt-0.5">
                          <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                          <p className="text-xs text-blue-600 truncate">{item.restaurant_name || 'ไม่มีชื่อร้าน'}</p>
                        </div>
                      </div>
                      <button
                        onClick={() => removeFromCart(item.product_id, item.restaurant_id)}
                    ...
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() => updateCartQuantity(item.product_id, item.restaurant_id, item.quantity - 1)}
              ...
              <div className="text-center py-6">
                ...
                <h3 className="text-sm font-medium text-gray-800 mb-1">ตะกร้าว่าง</h3>
                <p className="text-xs text-gray-500">เลือกเมนูที่ลูกค้าต้องการ</p>
              </div>
            )}
          </div>
          ...
          {cart.length > 0 && (
            <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-4">
              <div className="flex items-center space-x-2 mb-4">
                ...
                <div>
                  <h2 className="text-lg font-bold text-gray-800">📊 สรุปออร์เดอร์</h2>
                  <p className="text-xs text-gray-500">ตรวจสอบก่อนสั่งซื้อ</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="bg-white/60 rounded-lg p-3 space-y-2">
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-700">ยอดรวมสินค้า:</span>
                    <span className="text-lg font-bold text-gray-800">{formatPrice(calculateSubtotal())}</span>
                  </div>
                  
                  <div className="space-y-2">
                    <div className="flex items-center space-x-1">
                      <span className="text-sm text-gray-700 font-medium">ค่าจัดส่ง:</span>
                      <svg className="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <input
                      ...
                      step="0.01"
                          placeholder="0"
                        />
                        <span className="text-xs text-gray-700 font-medium">บาท</span>
                      </div>
                    </div>
                  </div>
                  
                  <div className="border-t pt-2 flex justify-between items-center">
                    <span className="text-base font-bold text-gray-800">ยอดชำระทั้งหมด:</span>
