...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      ...
      // กรณีต้อง verify email
      toast.error(translate("common.please_verify_email_before_logging_in"));
  ...
  const isRouteAllowed = (path, userRole) => {
    ...
    if (path.startsWith("/restaurant")) {
      return (
        userRole === "general_restaurant" || userRole === "special_restaurant"
      );
    }
  ...
  const getDashboardRoute = (role) => {
    switch (role) {
      ...
      default:
        ...
        const isCustomerRoute = customerAllowedRoutes.some(
          (route) =>
            from === route ||
            from.startsWith("/restaurants/") ||
            from.startsWith("/categories/")
        );
          ...
          <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  {translate("auth.username")} / {translate("auth.email")}
                </label>
                <input
                  id="username"
                  name="username"
                  ...
                  placeholder={`${translate("auth.username")} / ${translate("auth.email")}`}
                />
              </div>
